...
@torch.no_grad()
def calculate_fid_given_paths(paths, path_masks, attributes, img_size=256, batch_size=50):
    print('Calculating FID given paths %s and %s...' % (paths[0], paths[1]))
    device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
    inception = InceptionV3().eval().to(device)
    loaders = [get_eval_loader(path, path_masks, attributes, img_size, batch_size) for path in paths]
    mu, cov = [], []
